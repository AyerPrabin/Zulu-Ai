import os
# ===== ZULU AI UPGRADE CORE (MERGED) =====
# (Neural + Tools + GodMode in one safe block)
...
try:
    ...
    _collection = _client.get_or_create_collection("zulu")
    _NEURAL_OK = True
except:
    _NEURAL_OK = False
...
def archive_context(text, meta):
    if not _NEURAL_OK: return
    emb = _embed(text)
    if not emb: return
...
def query_memory(q):
    ...
    res = _collection.query(query_embeddings=[emb], n_results=3)
...
def run_tool(name, args):
    if name == "web":
        try:
            return requests.get(args.get("url"), timeout=5).text[:1000]
        except:
            return "web error"
...
# --- GodMode ---
def run_godmode(command, user_type="BOSS"):
    memory = query_memory(command)
...
"""
   ...
   if len(command) > 20:
    result = run_godmode(command, user_type)
else:
    result, _ = run_easy_flow(command, user_type)

    # tool execution
    if "[TOOL:" in result:
        try:
            tool_json = result.split("[TOOL:")[1].split("]")[0]
            tool_data = json.loads(tool_json)
            tool_result = run_tool(tool_data["name"], tool_data["args"])
            result += f"\nTool Result: {tool_result}"
        except:
            pass

    # archive memory async
    try:
        threading.Thread(
            target=archive_context,
            args=(command + result, {"type": "task"}),
            daemon=True
        ).start()
    except:
        pass
